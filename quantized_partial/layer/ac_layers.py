...
class ACBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:  ## MICCAI HCT
            ...
            residual = self.dequant0(residual)
            out = self.dequant0(out)

        # Self-attention using performers
        out = self.performer_attention(out)

        ## residual with input x
        # out += x
        out = self.float_add.add(out, x) # for quantization
        ...
        # out += residual
        out = self.float_add.add(out, residual) # for quantization
