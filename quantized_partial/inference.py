...
class InferenceEngine:
    ...
    def __init__(self, model:nn.Module, model_state_path:str, device:str, quantized=False) -> None:
        ...
        if quantized:
            backend = "x86"
            # only quantize conv and linear layers, which are largest size
            for n, m in self.model.named_modules():
                if ((isinstance(m, torch.nn.modules.conv.Conv2d) or 
                    isinstance(m, torch.nn.modules.linear.Linear) or
                    isinstance(m, torch.ao.quantization.stubs.QuantStub) or
                    isinstance(m, torch.ao.quantization.stubs.DeQuantStub)) and
                    n != "classifier" and n != "conv_stages.first_conv"): # dont quantize 1st and last layer
    ...
    def compile(self):
        ...
        with torch.no_grad():
            ...
            self.scripted_model_state_path = os.path.join(self.model_state_dir, "torchscript_hct.pth")
